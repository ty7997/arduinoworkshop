void setup() {
 ...
 // pinMode(4, OUTPUT); //LED connected to pin 4 so set as output
  pinMode(4, OUTPUT); //buzzer connected to pin 4 so set as output
  pinMode(A0, INPUT); //heat sensor connected to A0 so set as input
  pinMode(A1, INPUT); //heat sensor connected to A0 so set as input
...
Serial.println(light); //print out elsius to 

  if(light > 600){

    digitalWrite(6, HIGH); //turn LED connected to pin 4 on
    delay(2000); //wait for 2000 milliseconds
  }
  else if(light <= 600){
    
    digitalWrite(6,LOW); //turn LED connected to pin 4 off
    delay(2000); //wait for 2000 milliseconds
